//
//  CustomerMoviesViewController.swift
//  Cinemagic
//
...
// MARK: - CustomerMoviesViewController

class CustomerMoviesViewController: UITableViewController, UISearchResultsUpdating, EmptyDataSetSource, EmptyDataSetDelegate {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        guard let movieInfo = segue.destination as? CustomerMovieInfoViewController else { return }
        movieInfo.movie = (sender as? CustomerMovieCell)?.movie
    }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell: CustomerMovieCell = tableView.dequeue(for: indexPath).require()
    ...
    func description(forEmptyDataSet scrollView: UIScrollView) -> NSAttributedString? {
        ...
        if lastError != nil {
            ...
            title = "К сожалению, сейчас нет доступных к просмотру фильмов."
        }
