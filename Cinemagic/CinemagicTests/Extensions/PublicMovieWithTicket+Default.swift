//
//  PublicMovieWithTicket+Default.swift
//  CinemagicTests
//
...
// MARK: - PublicMovieWithTicket+Default
...
extension PublicMovieWithTicket {
    ...
    static var defaultMovies: [PublicMovieWithTicket] { [
        PublicMovieWithTicket(id: 8,
                    title: "Мстители",
                    description: """
...
""",
    ...
    static var unexistingMovie: PublicMovieWithTicket { PublicMovieWithTicket(id: 0, title: "Test movie", description: "Test movie description", showtime: dateFormatter.date(from: "2020-07-24")!) }
    static var movieWithoutTickets: PublicMovieWithTicket {
        PublicMovieWithTicket(id: 22,
                            title: "Железный человек",
                            description: """
Миллиардер-изобретатель Тони Старк попадает в плен к Афганским террористам, которые пытаются заставить его создать оружие массового поражения. В тайне от своих захватчиков Старк конструирует высокотехнологичную киберброню, которая помогает ему сбежать. Однако по возвращении в США он узнаёт, что в совете директоров его фирмы плетётся заговор, чреватый страшными последствиями. Используя своё последнее изобретение, Старк пытается решить проблемы своей компании радикально…
